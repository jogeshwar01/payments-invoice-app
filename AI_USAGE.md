...
I have used AI heavily on this one. I drove a Claude Code session in the terminal - explained what
the assignment was, talked through the design out loud, and had it produce
files. The architectural calls (two-transaction payment flow
...
The sections below are the ones explicitly asked for. I wrote the
intent for each; the prose was drafted with AI and edited by me. Where I say
"I rejected X" or "I caught Y".those are real moments from the session. After this the content is this file is drafted by me, but refined with AI.

---

## This next part is AI Written (I drafted the intent)
...
- **Claude (via Claude Code CLI)** - the main driver. Used for:
  - Drafting the initial sqlx migrations after I described the table list.
    ...
    because that uniqueness is _the_ idempotency primitive, not a
    nice-to-have.
  ...
  - Doctest-block fixes when rustdoc tried to compile my ASCII diagrams as
    Rust.
- **No Cursor / Copilot / ChatGPT** on this one.

What I didn't use AI for: the state machine design, the two-transaction
flow, the choice to key the PSP by our attempt_id, and the cut list in DESIGN.md §6. Those came out of thinking
...
I also asked Claude to use a row lock
on `invoices` is self-documenting - anyone reading the code knows what's
being serialized.

### 2.2. Mock PSP keyed by _our_ attempt_id, not by a PSP-generated id
...
`attempt_id` as the _idempotency key_ on the PSP side. The PSP stores the
