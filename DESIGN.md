...
*between* two transactions, with no DB lock held during network I/O.
...
- **Optimistic concurrency on a version column** would work too, and we keep
  ...
  *blocks* rather than racing-then-retrying.
...
- *Different idempotency keys.* They contend on `SELECT … FOR UPDATE
...
- *Same idempotency key.* Both pass the lock acquisition (in serial), but
...
*new* `attempt_id` from a new `/pay` call, which would block on the row
...
We deliberately do *not* silently treat the new body as the original.
...
3. **Audit log.** Right now we have row history but no record of *who*
