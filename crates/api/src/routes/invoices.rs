...
pub fn scope() -> Scope {
    web::scope("/invoices")
        ...
        .route(
            "/{id}/pay",
            web::post().to(crate::routes::payments::pay_handler),
        )
}
...
async fn create(
    ...
    if req.line_items.is_empty() {
        return Err(ApiError::BadRequest(
            "at least one line item required".into(),
        ));
    }
    ...
    for li in &req.line_items {
        ...
        if li.description.trim().is_empty() {
            return Err(ApiError::BadRequest(
                "line item description required".into(),
            ));
        }
    ...
    if owner.is_none() {
        return Err(ApiError::BadRequest(
            "customer not found for business".into(),
        ));
    }
...
async fn load_line_items(state: &AppState, invoice_id: Uuid) -> Result<Vec<LineItem>, ApiError> {
    ...
    Ok(rows
        .into_iter()
        .map(
            |(id, description, quantity, unit_amount_cents, position)| LineItem {
                id,
                description,
                quantity,
                unit_amount_cents,
