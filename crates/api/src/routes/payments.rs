...
async fn run_tx_a(
    ...
    let existing: Option<(Uuid, Vec<u8>, String, Option<String>, Option<String>, DateTime<Utc>, Option<DateTime<Utc>>)> =
        sqlx::query_as(
    ...
    let cur: InvoiceState = cur_state
        .parse()
...
async fn read_existing(
    ...
    let row: Option<(Uuid, String, Option<String>, Option<String>, DateTime<Utc>, Option<DateTime<Utc>>)> =
        sqlx::query_as(
    ...
    match psp_res {
        Ok(PspOutcome::Succeeded { psp_ref }) => {
            finalize_tx_b(
                ...
                invoice_id,
                attempt_id,
                TxBOutcome::Success { psp_ref: psp_ref.clone() },
            )
            .await?;
...
pub(crate) async fn finalize_tx_b(
    ...
    let cur_attempt: Option<(String,)> = sqlx::query_as(
    ...
    match &outcome {
        TxBOutcome::Success { psp_ref } => {
            ...
            if cur == InvoiceState::Processing {
                let next = try_transition(cur, TransitionEvent::PspSuccess).map_err(|e| {
                    ApiError::Internal(format!("unexpected transition: {e}"))
                })?;
                sqlx::query(
        ...
        TxBOutcome::Failed { code } => {
            if cur == InvoiceState::Processing {
                let next = try_transition(cur, TransitionEvent::PspFailure).map_err(|e| {
                    ApiError::Internal(format!("unexpected transition: {e}"))
                })?;
                sqlx::query(
                    "UPDATE invoices SET state = $1, updated_at = now() WHERE id = $2",
...
async fn load_attempt(
    ...
    let row: (String, Option<String>, Option<String>, DateTime<Utc>, Option<DateTime<Utc>>) =
        sqlx::query_as(
